...
d.addEventListener("keypress",async (e)=>{
    if(e.target.matches("#search")){
        if(e.key === "Enter"){
            try {
                $shows.innerHTML=`<img class="loader" src="../../public/assets/svg/puff.svg" alt="Cargando...">`
                ...
                if(json.length===0){
                    $shows.innerHTML = `<h2>No existe resultado de shows para el criterio de busqueda: <mark>${query}</mark></h2>`
                }else{
                    json.forEach(el => {
